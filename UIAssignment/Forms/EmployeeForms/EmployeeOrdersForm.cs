...
{
    public partial class EmployeeOrdersForm : ChildForm
    {
        List<Order> orders = ActiveUser.Employee.Orders;
        ...
        public EmployeeOrdersForm()
        {
            ...
            if (orderSectionCount + 5 >= orders.Count)
                nextOrderSectionButtonIcon.Enabled = false;
        }
        ...
        private void loadAllOrdersSection(int startingPoint)
        {
            ...
            foreach (Order order in orders)
            {
                ...
                else if (counter >= startingPoint && counter < startingPoint + 5)
                {
                    ...
                    setOrderDetailSection(counter % 5, order, false);
                    counter++;
                    continue;
                }
                break;
            ...
            for (int i = counter; i < startingPoint + 5; i++)
                setOrderDetailSection(i % 5, null, true);
        }
        ...
        private void setOrderDetailSection(int sectionNumber, Order order, bool doNotFill)
        {
            ...
            panel.Controls.OfType<Label>().FirstOrDefault(label => label.Name.Contains("OrderUserUsernameValueLabel")).Text = order.CustomerName;
        ...
        private void completeIconOne_Click(object sender, EventArgs e)
        {
            trashIconButtonStandardActions(0);
        }

        private void completeIconTwo_Click(object sender, EventArgs e)
        {
            trashIconButtonStandardActions(1);
        }

        private void completeIconThree_Click(object sender, EventArgs e)
        {
            trashIconButtonStandardActions(2);
        }

        private void completeIconFour_Click(object sender, EventArgs e)
        {
            trashIconButtonStandardActions(3);
        }

        private void completeIconFive_Click(object sender, EventArgs e)
        {
            trashIconButtonStandardActions(4);
        }

        private async Task trashIconButtonStandardActions(int sectionNumber)
        {
            MiniLoadingForm loadingForm = new MiniLoadingForm();
            loadingForm.Size = new Size(480, 420);
            ...
            OrderDataAccess.DeleteOrder(ActiveUser.Employee.Orders[sectionNumber + orderSectionCount].CustomerName,
                ActiveUser.Employee.Orders[sectionNumber + orderSectionCount].IssuedDate);
            orders.Remove(ActiveUser.Employee.Orders[sectionNumber + orderSectionCount]);
            ...
            if (orderSectionCount + 5 < ActiveUser.Employee.Orders.Count)
